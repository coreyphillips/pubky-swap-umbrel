...
// Loopback inside the container, so nothing else on Umbrel's shared app network can reach it.
// Settable only so a developer running the app outside a container can move it off a port they
// already use; there is nothing to collide with in the container itself.
const STATUS_HOST = process.env.SWAP_STATUS_HOST || '127.0.0.1';
const STATUS_PORT = parseInt(process.env.SWAP_STATUS_PORT || '9737', 10);
